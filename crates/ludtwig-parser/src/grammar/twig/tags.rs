...
fn parse_twig_embed(
  ...
  let expected_end_tag = if is_sw_embed {
    ...
    if parser.at(expected_end_tag) {
        parser.bump();
    } else {
        if is_sw_embed {
            parser.add_error(ParseErrorBuilder::new("sw_end_embed"));
        } else {
            parser.add_error(ParseErrorBuilder::new("endembed"));
        }
        parser.recover(&[T!["%}"], T!["</"]]);
    }
    parser.expect(T!["%}"], &[T!["</"]]);
                ...
                ROOT@0..52
                  TWIG_EMBED@0..52
                    ...
                    TWIG_EMBED_ENDING_BLOCK@34..52
                      TK_CURLY_PERCENT@34..36 "{%"
                      TK_WHITESPACE@36..37 " "
                      TK_END_SW_EMBED@37..49 "sw_end_embed"
                      TK_WHITESPACE@49..50 " "
                      TK_PERCENT_CURLY@50..52 "%}""#]],
            );
        }
